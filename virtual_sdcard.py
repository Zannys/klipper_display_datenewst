...
class VirtualSD:
    ...
    def get_file_list(self, check_subdirs=False, sort_by_date_reverse=False):
        if check_subdirs:
            flist = []
            try:
                ...
                if sort_by_date_reverse:
                    # ordenar por mtime (mais novos primeiro)
                    def _mtime_tuple(item):
                        rel_path, _ = item
                        abs_path = os.path.join(self.sdcard_dirname, rel_path)
                        try:
                            return os.path.getmtime(abs_path)
                        except Exception:
                            return 0.0
                    return sorted(flist, key=_mtime_tuple, reverse=True)
                return sorted(flist, key=lambda f: f[0].lower())
            except:
                logging.exception("virtual_sdcard get_file_list (recursive)")
                raise self.gcode.error("Unable to get file list")
        else:
            dname = self.sdcard_dirname
            try:
                filenames = os.listdir(self.sdcard_dirname)
                # só arquivos “visíveis” e regulares
                entries = [fname for fname in filenames
