...
class MenuElement(object):
    def __init__(self, manager, config, **kwargs):
        ...
        # >>> ADD: flag de ordenação por data (mais novos primeiro)
        self._newfirst = kwargs.get('newfirst', False)
        # <<<
        if config is not None:
            ...
            try:
                ...
                self._enable_tpl = manager.gcode_macro.load_template(
                    config, 'enable')
            # >>> ADD: ler newfirst do config se existir
            try:
                self._newfirst = config.getboolean('newfirst', self._newfirst)
            except config.error:
                pass
            # <<<
...
class MenuVSDList(MenuList):
    ...
    def _populate(self):
        ...
        if sdcard is not None:
            # >>> ALTERAÇÃO: repassa a flag newfirst como sort_by_date_reverse
            files = sdcard.get_file_list(sort_by_date_reverse=self._newfirst)
